...
/****************************************************************************** 
function:delly and lumpy share the number of deletions in hedou12 data. 
...
int main(void)
{
	...
	*/ 
	...
	if((delly_data=fopen("F://data//hedou//delly//deletion.txt","r"))==NULL){
		puts("DELLY data file can not open.");
		exit(0);
	}
	
	if((lumpy_data=fopen("F://data//hedou//lumpy//deletion.txt","r"))==NULL){
	...
	*/
	/* 
	if((analysis_delly=fopen("F://data//delly_analysis_deletion.txt","w+"))==NULL){
		puts("delly_analysis_deletion can not open.");
		exit(0);
	} 
	*/ 
	if((total_file=fopen("F://data//hedou//dl_total_deletion.txt","w+"))==NULL){
		puts("dl_total_deletion can not open.");
		exit(0);
	} 
	//extract file
	char line[1024];
...
//	long int delly_count=0;
	...
	*/
	...
	while(fgets(line,1024,delly_data)!=NULL){ 
		...
		for(long int j=0;j<delly_count;j++){
			...
			&&((delly_array[j].length==lumpy_array[i].length==delly_array[z].length)))){
			 ...
			 if((delly_array[j].length>=1)&&(delly_array[j].length<=499)) deletion_1++;
			  	if((delly_array[j].length>=500)&&(delly_array[j].length<=999)) deletion_500++;
			  	if((delly_array[j].length>=1000)&&(delly_array[j].length<=1999)) deletion_1000++;
			  	if((delly_array[j].length>=2000)&&(delly_array[j].length<=2999)) deletion_2000++;
			  	if((delly_array[j].length>=3000)&&(delly_array[j].length<=3999)) deletion_3000++;
			  	if((delly_array[j].length>=4000)&&(delly_array[j].length<=4999)) deletion_4000++;
			  	if((delly_array[j].length>=5000)&&(delly_array[j].length<=5999)) deletion_5000++;
			  	if(delly_array[j].length>=6000) deletion_6000++;
			  }
	}
		}
	...
	for(long int i=0;i<delly_count;i++){
		for(long int j=0;j<delly_count;j++){
				//printf("%ld\t%ld\t%ld\n",delly_array[j].start,delly_array[j].end,delly_array[j].length);
			if(((delly_array[j].start>=(delly_array[i].start-20))&&(delly_array[j].start<=(delly_array[i].start+20))&&((delly_array[j].length==delly_array[i].length)))
			  ||((delly_array[j].end>=(delly_array[i].end-20))&&(delly_array[j].end<=(delly_array[i].end+20))&&((delly_array[j].length==delly_array[i].length)))){
			  	fprintf(total_file,"%ld\t%ld\t%ld\n",delly_array[j].start,delly_array[j].end,delly_array[j].length);
