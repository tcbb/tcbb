...
/****************************************************************************** 
function: PASA,delly and lumpy share the number of inverions in Hedou12 
...
int main(void)
{
	...
	if((analysis_delly=fopen("F://data//hedou///delly_analysis_inversion.txt","w+"))==NULL){
		puts("delly_analysis_inversion can not open.");
		exit(0);
	}
	if((analysis_lumpy=fopen("F://data//hedou//umpy_analysis_inversion.txt","w+"))==NULL){
		puts("lumpy_analysis_inversion can not open.");
		exit(0);
	}
	if((analysis_pasa=fopen("F://data/hedou//pasa_analysis_inversion.txt","w+"))==NULL){
		puts("pasa_analysis_inversion can not open.");
		exit(0);
	} 
	if((total_file=fopen("F://data//hedou//pdl_total_inversion.txt","w+"))==NULL){
		puts("pdl_total_inversion can not open.");
		exit(0);
	} 
	//extract file
	char line[1024];
    ...
    */
	...
	while(fgets(line,1024,delly_data)!=NULL){ 
		...
		for(long int j=0;j<pasa_count;j++){
			...
			if(((pasa_array[j].start>=(lumpy_array[i].start-35))&&(pasa_array[j].start<=(lumpy_array[i].start+35))
			 &&(pasa_array[j].start>=(delly_array[z].start-35))&&(pasa_array[j].start<=(delly_array[z].start+35))
			 &&(delly_array[z].start>=(lumpy_array[i].start-35))&&(delly_array[z].start<=(lumpy_array[i].start+35))
			 &&((pasa_array[j].length==lumpy_array[i].length)==delly_array[z].length))
			||((pasa_array[j].end>=(lumpy_array[i].end-35))&&(pasa_array[j].end<=(lumpy_array[i].end+35))
			&&(delly_array[z].end>=(lumpy_array[i].end-35))&&(delly_array[z].end<=(lumpy_array[i].end+35))
			&&(pasa_array[j].end>=(delly_array[z].end-235))&&(pasa_array[j].end<=(delly_array[z].end+35))
			&&((pasa_array[j].length==lumpy_array[i].length==delly_array[z].length)))){
	...
	for(long int i=0;i<lumpy_count;i++){
		for(long int j=0;j<pasa_count;j++){
				//printf("%ld\t%ld\t%ld\n",pasa_array[j].start,pasa_array[j].end,pasa_array[j].length);
			if(((pasa_array[j].start>=(lumpy_array[i].start-30))&&(pasa_array[j].start<=(lumpy_array[i].start+30))&&((pasa_array[j].length==lumpy_array[i].length)))
			  ||((pasa_array[j].end>=(lumpy_array[i].end-30))&&(pasa_array[j].end<=(lumpy_array[i].end+30))&&((pasa_array[j].length==lumpy_array[i].length)))){
	...
	for(long int i=0;i<pasa_count;i++){
		for(long int j=0;j<delly_count;j++){
			...
			if(((delly_array[j].start>=(pasa_array[i].start-25))&&(delly_array[j].start<=(pasa_array[i].start+25))&&((delly_array[j].length==pasa_array[i].length)))
			  ||((delly_array[j].end>=(pasa_array[i].end-25))&&(delly_array[j].end<=(pasa_array[i].end+25))&&((delly_array[j].length==pasa_array[i].length)))){
