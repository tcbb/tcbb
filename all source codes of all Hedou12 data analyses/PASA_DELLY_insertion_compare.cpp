...
/****************************************************************************** 
function:PASA and delly share the number of insertions in Hedou12
...
int main(void)
{
	puts("start");
	puts("....");
	FILE *pasa_data,*delly_data,*total_file;
	/* 
	...
	*/ 
	if((pasa_data=fopen("F://data//hedou//pasa//insertion.txt","r"))==NULL){
		puts("PASA data file can not open.");
		exit(0);
	}
	
	if((delly_data=fopen("F://data//hedou//delly//insertion.txt","r"))==NULL){
		puts("DELLY data file can not open.");
		exit(0);
	}
	
	
	if((total_file=fopen("F://data//hedou//pd_total_insertion.txt","w+"))==NULL){
		puts("pd_total_insertion can not open.");
		exit(0);
	} 
	...
	while(fgets(line,1024,delly_data)!=NULL){ 
		...
		*/
		...
		for(long int j=0;j<delly_count;j++){
			...
			if(((pasa_array[i].start>=(delly_array[j].start-25))&&(pasa_array[i].start<=(delly_array[j].start+25))
			...
			||((pasa_array[i].end>=(delly_array[j].end-25))&&(pasa_array[i].end<=(delly_array[j].end+25))
			&&(pasa_array[i].end>=(lumpy_array[z].end-25))&&(pasa_array[i].end<=(delly_array[z].end+25))
