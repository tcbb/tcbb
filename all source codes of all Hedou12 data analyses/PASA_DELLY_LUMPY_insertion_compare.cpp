...
/****************************************************************************** 
function:PASA and delly and lumpy share the number of insertions in Hedou12 
...
int main(void)
{
	...
	*/ 
	if((pasa_data=fopen("F://data//hedou12//pasa//insertion.txt","r"))==NULL){
		puts("PASA data file can not open.");
		exit(0);
	}
	
	if((delly_data=fopen("F://data//hedou12//delly//insertion.txt","r"))==NULL){
		puts("DELLY data file can not open.");
		exit(0);
	}
	if((lumpy_data=fopen("F://data//hedou12//lumpy//insertion.txt","r"))==NULL){
	...
	if((analysis_delly=fopen("F://data//hedou12//insertion_total.txt","w+"))==NULL){
		puts("delly_analysis_insertion can not open.");
		exit(0);
	}
	if((analysis_lumpy=fopen("F://data//hedou12//lumpy_analysis_insertion.txt","w+"))==NULL){
		puts("lumpy_analysis_insertion can not open.");
		exit(0);
	}
	if((analysis_pasa=fopen("F://data//hedou12//pasa_analysis_insertion.txt","w+"))==NULL){
		puts("pasa_analysis_insertion can not open.");
		exit(0);
	} 
	if((total_file=fopen("F://data//hedou12//pdl_total_insertion.txt","w+"))==NULL){
		puts("pdl_total_insertion can not open.");
		exit(0);
	} 
	...
	while(fgets(line,1024,delly_data)!=NULL){ 
		...
		for(long int j=0;j<delly_count;j++){
			...
			if(((pasa_array[i].start>=(delly_array[j].start-30))&&(pasa_array[i].start<=(delly_array[j].start+30))
			&&(pasa_array[i].start>=(lumpy_array[z].start-30))&&(pasa_array[i].start<=(lumpy_array[z].start+30))
			&&(delly_array[j].start>=(lumpy_array[z].start-30))&&(delly_array[j].start<=(lumpy_array[z].start+30))
			&&((pasa_array[j].length==delly_array[i].length==lumpy_array[z].length)))
			||((pasa_array[i].end>=(delly_array[j].end-30))&&(pasa_array[i].end<=(delly_array[j].end+30))
			&&(pasa_array[i].end>=(lumpy_array[z].end-30))&&(pasa_array[i].end<=(delly_array[z].end+30))
			&&(lumpy_array[z].end>=(delly_array[i].end-30))&&(lumpy_array[z].end<=(delly_array[i].end+30))
			&&((pasa_array[j].length==delly_array[i].length)==lumpy_array[z].length))){
...
fprintf(total_file,"length(1bp-99bp):%ld\nlength(100bp-199bp):%ld\nlength(200-300bp):%ld\n",insertion_1,insertion_100,insertion_200);
		//||((pasa_array[j].length>=(simulate_array[i].length-25))&&(pasa_array[j].length<=(simulate_array[i].length+30))) 

	for(long int i=0;i<delly_count;i++){
		for(long int j=0;j<pasa_count;j++){
				//printf("%ld\t%ld\t%ld\n",pasa_array[j].start,pasa_array[j].end,pasa_array[j].length);
			if(((pasa_array[j].start>=(delly_array[i].start-30))&&(pasa_array[j].start<=(delly_array[i].start+30))&&((pasa_array[j].length==delly_array[i].length)))
			  ||((pasa_array[j].end>=(delly_array[i].end-30))&&(pasa_array[j].end<=(delly_array[i].end+30))&&((pasa_array[j].length==delly_array[i].length)))){
