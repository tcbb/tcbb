...
#include <string.h>
/****************************************************************************** 
function:compare PASA with delly and lumpy,separately.
...
int main(void)
{
	...
	if((simulate_data=fopen("F:\\data\\insertion.txt","r"))==NULL){
		puts("simulate data can not open.");
		exit(0);
	}
	if((pasa_data=fopen("F:\\data\\PASA_insertion.txt","r"))==NULL){
		puts("PASA data file can not open.");
		exit(0);
	}
	
	if((delly_data=fopen("F:\\data\\DELLY_insertion.txt","r"))==NULL){
		puts("DELLY data file can not open.");
		exit(0);
	}
	if((lumpy_data=fopen("F:\\data\\LUMPY_insertion.txt","r"))==NULL){
