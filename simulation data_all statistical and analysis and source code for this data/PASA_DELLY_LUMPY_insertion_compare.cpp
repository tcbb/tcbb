...
/****************************************************************************** 
function:PASA and delly and lumpy share the number of insertions in simulation data. 
...
int main(void)
{
	...
	*/ 
	...
	if((total_file=fopen("F://data//simulation//pdl_total_insertion.txt","w+"))==NULL){
		puts("pasa_analysis_insertion can not open.");
		exit(0);
	} 
	
	
	if((analysis_delly=fopen("F://data//simulation//insertion_total.txt","w+"))==NULL){
...
fprintf(total_file,"length(1bp-99bp):%ld\nlength(100bp-199bp):%ld\nlength(200-300bp):%ld\n",insertion_1,insertion_100,insertion_200);
	...
	for(long int i=0;i<delly_count;i++){
		for(long int j=0;j<pasa_count;j++){
				//printf("%ld\t%ld\t%ld\n",pasa_array[j].start,pasa_array[j].end,pasa_array[j].length);
			if(((pasa_array[j].start>=(delly_array[i].start-25))&&(pasa_array[j].start<=(delly_array[i].start+25))&&((pasa_array[j].length==delly_array[i].length)))
			  ||((pasa_array[j].end>=(delly_array[i].end-25))&&(pasa_array[j].end<=(delly_array[i].end+25))&&((pasa_array[j].length==delly_array[i].length)))){
