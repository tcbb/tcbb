...
running environment:ubuntu 14.0.2 server operating system
...
int main(void)
{
	...
	if((simulate_data=fopen("F:\\data\\deletion.txt","r"))==NULL){
		puts("simulate data can not open.");
		exit(0);
	}
	if((pasa_data=fopen("F:\\data\\PASA_deletion.txt","r"))==NULL){
		puts("PASA data file can not open.");
		exit(0);
	}
	
	if((delly_data=fopen("F:\\data\\DELLY_deletion.txt","r"))==NULL){
		puts("DELLY data file can not open.");
		exit(0);
	}
	if((lumpy_data=fopen("F:\\data\\LUMPY_deletion.txt","r"))==NULL){
	...
	if((analysis_lumpy=fopen("F:\\data\\lumpy_analysis_deletion.txt","w+"))==NULL){
