...
int main(void)
{
	...
	if((simulate_data=fopen("F://data//similation_deletion.txt","r"))==NULL){
		puts("simulate data can not open.");
		exit(0);
	}
	if((pasa_data=fopen("F://data//extract_PASA_deletion.txt","r"))==NULL){
		puts("PASA data file can not open.");
		exit(0);
	}
	
	if((delly_data=fopen("F://data//extract_DELLY_deletion.txt","r"))==NULL){
		puts("DELLY data file can not open.");
		exit(0);
	}
	if((lumpy_data=fopen("F://data//extract_LUMPY_deletion.txt","r"))==NULL){
	...
	if((analysis_lumpy=fopen("F://data//umpy_analysis_deletion.txt","w+"))==NULL){
